    ...
    useEffect(() => {
        ...
        const step = () => {
            ...
            setRound(prev => {
                ...
                if (action === 'hold') {
                    const botNext = { ...prev.bot, held: true, turnIndex: Math.max(prev.bot.turnIndex, 1) };
                ...
                const botNext = { player: 'bot', turnIndex: prev.bot.turnIndex + 1, rollSum: r.sum, held: false, dice: [r.dieA, r.dieB] };
                return { ...prev, bot: botNext };
            });
            setTimeout(step, 600);
        };
