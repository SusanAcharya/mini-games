...
type RoundState = {
  human: TurnState
  bot: TurnState
  stage: 'playing' | 'finished' | 'all-in'
  winner: 'human' | 'bot' | 'tie' | null
  currentTurn: Player
}

function rollTwoDice(): { dieA: number; dieB: number; sum: number } {
  const dieA = 1 + Math.floor(Math.random() * 6)
  const dieB = 1 + Math.floor(Math.random() * 6)
  return { dieA, dieB, sum: dieA + dieB }
}

function decideBotAction(currentSum: number | null, turnIndex: number): 'hold' | 'reroll' {
  const sum = currentSum ?? 0
  if (sum >= 10) return 'hold'
  if (turnIndex >= 3) return 'hold'
  if (sum >= 9) {
...
function useAudio() {
  ...
  const blip = (freq: number, duration = 0.08, type: OscillatorType = 'square') => {
    const ctx = ensure()
...
export function DiceRisk(): JSX.Element {
  ...
  const isPlayerDone = (t: TurnState) => t.held || t.turnIndex >= 3
  ...
  const statusText = useMemo(() => {
    if (round.stage === 'finished') {
      if (round.winner === 'human') return 'You win! +20 points'
      if (round.winner === 'bot') return 'You lost. 0 points'
  ...
  const resolveWinner = (human: TurnState, bot: TurnState) => {
    const h = human.rollSum ?? 0
    const b = bot.rollSum ?? 0
    if (h > b) return 'human' as const
    if (b > h) return 'bot' as const
    return 'tie' as const
  }

  const isRoundOver = (state: RoundState) => isPlayerDone(state.human) && isPlayerDone(state.bot)
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
      <div>
        <Panel title="You">
          <ScoreRow label="Roll" value={round.human.rollSum ?? 0} highlight={true} shakeKey={shakeKey} dice={round.human.dice} />
          <TurnsRow current={round.human.turnIndex} held={round.human.held} />
          <div style={{ display: 'flex', gap: 12, marginTop: 12 }}>
            <PrimaryButton onClick={doHumanRoll} disabled={!canHumanAct}>Roll</PrimaryButton>
            <SecondaryButton onClick={doHumanHold} disabled={!canHumanAct}>Hold</SecondaryButton>
            <TertiaryButton onClick={reset}>Reset</TertiaryButton>
          </div>
        </Panel>
      </div>
      <div>
        <Panel title="Bot">
          <ScoreRow label="Roll" value={round.bot.rollSum ?? 0} shakeKey={shakeKey} dice={round.bot.dice} />
          <TurnsRow current={round.bot.turnIndex} held={round.bot.held} />
        </Panel>
      </div>
        ...
        <Panel title="Round Status">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <PrimaryButton onClick={doAllIn} disabled={!canAllIn}>All-In Final Roll</PrimaryButton>
              {canTieOptions && (
                <>
                  <SecondaryButton onClick={doTieSplit}>Split Points</SecondaryButton>
                  <PrimaryButton onClick={doTieGamble}>Tie Gamble</PrimaryButton>
                </>
              )}
            </div>
          </div>
...
function ScoreRow({ label, value, dice, highlight, shakeKey }: { label: string; value: number; dice?: [number, number] | null; highlight?: boolean; shakeKey?: number }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      <div style={{ opacity: 0.8 }}>{label}</div>
      <DiceVisual value={value} dice={dice ?? undefined} shakeKey={shakeKey} />
      <div style={{ fontSize: 28, fontWeight: 800, color: highlight ? '#8be9a5' : '#eaeaf0', textShadow: '0 2px 12px rgba(139,233,165,0.2)' }}>{value}</div>
    </div>
  )
}

function TurnsRow({ current, held }: { current: number; held: boolean }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
      <div style={{ display: 'flex', gap: 6 }}>{circles}</div>
      {held && <span style={{ marginLeft: 8, color: '#f7c948' }}>Held</span>}
    </div>
  )
}
...
function TertiaryButton({ children, onClick, disabled }: { children: React.ReactNode; onClick?: () => void; disabled?: boolean }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{
...
function DiceVisual({ value, dice, shakeKey }: { value: number; dice?: [number, number]; shakeKey?: number }) {
...
function Die({ pip }: { pip: number }) {
  const spots = Array.from({ length: pip }).map((_, i) => (
